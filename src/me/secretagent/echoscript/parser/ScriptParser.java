...
public class ScriptParser {
	...
	public void run() throws FileNotFoundException {
		System.out.println("Running " + script.getName() + "...");
		...
		System.out.println("Ran " + script.getName());
	}
	...
	private void onLine(String s, int line) {
		...
		if (s.startsWith("var")) {
				...
				for (ScriptMethod method : clazz.getMethods()) {
					if (methodstr.startsWith(method.getName())) {
						if (methodstr.startsWith(method.getName())) {
							...
							try {
								ScriptVariable.vars.put(strings[1], method.onInvoked(args));
								return;
							} catch (Exception e) {
			...
			try {
				...
				ScriptVariable.vars.put(strings[1], ScriptString.parse(ss));
				return;
			} catch (Exception e) {
				
			}
			String equation = s.substring(s.indexOf("=") + 1);
			ScriptVariable.vars.put(strings[1], MathUtil.parseEquation(equation));
			return;
			...
			for (String liness : lines) {
				if (liness.equals("}") && i > line) {
					methodlines.add(i);
					ScriptFunctions.functions.put(funcname, methods);
					i++;
					return;
				}
				if (i > line) {
			...
			if (!new File(folder, name).exists()) {
				...
				ScriptImportParser parser = new ScriptImportParser(importfile, ogname);
				try {
					parser.run();
